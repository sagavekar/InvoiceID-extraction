...
def getcXML():
    ...
    print(df.head())
    df.to_excel(f"{client_name}_cXML_InvoiceData_{datetime.today().date()}.xlsx", index=False)

    os.startfile(f"{client_name}_cXML_InvoiceData_{datetime.today().date()}.xlsx") # this approach to open excel file is not reliable or cross-platform compatible.
...
def getEDI():
    ...
    def process_folders(base_path,client_name):
        ...
        if data:
            df = pd.DataFrame(data)
            print(df.head())
            df.to_excel(f"{client_name}_EDI_InvoiceData_{datetime.today().date()}.xlsx", index=False)
            print("Data saved , pls check parent folder! ")
            os.startfile(f"{client_name}_EDI_InvoiceData_{datetime.today().date()}.xlsx") # this approach is not reliable to open excel file or cross-platform compatible.
        else:
            print("No EDI files found or no invoice numbers extracted.")

    process_folders(base_path,client_name) # calling process_folder  in order to exc. above two functions
...
# Create main window
root = tk.Tk()
root.title("Read Invoice | Dev: Omkar Sagavekar-TSO")

# Invoice format selection (radio buttons)
invoice_format = tk.IntVar()  # Integer variable to store selection (1 for cXML, 2 for EDI)
...
process_button = tk.Button(root, text="Process", command=lambda: process_invoices(invoice_format.get()))
